...
void solve_groundwater_system(Data **data, Map *gmap, QMatrix A, Vector b, Vector x, Config *param)
{
    ...
    if (param->iter_solve == 0) {
        ...
        for (irow = 0; irow < gmap->nactv; irow++) {
            ...
            // printf(" -%d- : (%f, %f, %f) %f = (%f)\n",ii,(*data)->Gzm[ii],(*data)->Gct[ii],
            //     (*data)->Gzp[ii],(*data)->h_incr[ii],-(*data)->resi[ii]);
    ...
    for (jj = 0; jj < param->n3ci; jj++)
    {
        if (gmap->actv[jj] == 1 & gmap->istop[jj] == 1)
        {
            ...
            if (param->sim_shallowwater == 1)
            {
                ...
                // if ((*data)->Asz[gmap->top2d[jj]] > 0.0)    {
                //     (*data)->qseepage[gmap->top2d[jj]] += (*data)->qz[ii]/(*data)->Asz[gmap->top2d[jj]];
                // }
                // else {
                //     (*data)->qseepage[gmap->top2d[jj]] += (*data)->qz[ii]/gmap->Az[jj];
                // }
                if (gmap->Az[jj] > 0.0) {
                    (*data)->qseepage[gmap->top2d[jj]] += (*data)->qz[ii]/gmap->Az[jj];
                }
                ...
                (*data)->qss[gmap->top2d[jj]] = (*data)->qseepage[gmap->top2d[jj]] * (*data)->r_rhozp[ii];
...
{
    int ii, jj, i_sat;
    double coeff, dqx, dqy, dqz, room_col, dz_eff, dqep;
    if (param->iter_solve == 0) {
        for (ii = 0; ii < param->n3ci; ii++)
        {
            if (gmap->actv[ii] == 1)    {
                // update water content
                // if ((*data)->wcn[ii] < (*data)->wcs[ii])    {
                //     coeff = gmap->V[ii] * (*data)->r_rho[ii];
                // }
                // else {
                ...
                dqep = 0.0;
                // if (gmap->istop[ii] == 1)   {
                //     if ((*data)->qtop[gmap->top2d[ii]] != 0.0)
                //     {
                //         dqep -= (*data)->qtop[gmap->top2d[ii]] * gmap->Az[ii] * param->dtg;
                //     }
                // }

                (*data)->wc[ii] = ((*data)->wcn[ii]*(*data)->r_rho[ii]*gmap->V[ii] + dqx + dqy + dqz + dqep) / coeff;
            }
        }
    }
    else {
        ...
        if (gmap->actv[ii] == 1)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    dV_tot += dV;
                    // if (gmap->istop[ii] == 1)   {(*data)->h[ii] = (*data)->hwc[ii];}
                    // else    {(*data)->wc[ii] = (*data)->wch[ii];}
...
void volume_by_flux_subs(Data **data, Map *gmap, Config *param)
{
    ...
    for (ii = 0; ii < param->n3ci; ii++)
    {
      (*data)->Vgflux[ii] = (*data)->Vg[ii] + param->dtg *
        ...
        // if (gmap->istop[ii] == 0 | param->sim_shallowwater == 0)    {
        //     (*data)->Vgflux[ii] -= param->dtg * (*data)->qz[gmap->icjckM[ii]];
        // }
        // else {
        //     if ((*data)->Asz[gmap->top2d[ii]] > 0.0) {
        //         (*data)->Vgflux[ii] -= param->dtg * (*data)->qz[gmap->icjckM[ii]] * (*data)->Asz[gmap->top2d[ii]] / gmap->Az[ii];
        //     }
        //     else {(*data)->Vgflux[ii] -= param->dtg * (*data)->qz[gmap->icjckM[ii]];}
        // }
    }
}
