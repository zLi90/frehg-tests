...
{
    int ii, jj, ll, aa;
    double sip, sim, sjp, sjm, s_lim_hi, s_lim_lo, s_rainevap, Vre, s_tmp;
    double *s_min, *s_max;
    s_lim_hi = 360.0;
    s_lim_lo = 0.0;
    ...
    for (aa = 0; aa < param->nactv; aa++)
    {
        ...
        // if (smap->jj[ii] == 35)  {
        //     printf(" BEGIN : (eta=%f, d=%f, s=%f) -%f-  (eta=%f, d=%f, s=%f, sm=%f) -%f- (eta=%f, d=%f, s=%f)\n",
        //         (*data)->eta[ii-1],(*data)->dept[ii-1],(*data)->s_surf[kk][ii-1],(*data)->vv[smap->icjM[ii]],
        //         (*data)->eta[ii],(*data)->dept[ii],(*data)->s_surf[kk][ii],(*data)->sm_surf[kk][ii],
        //         (*data)->vv[ii],(*data)->eta[ii+1],(*data)->dept[ii+1],(*data)->s_surf[kk][ii+1]);
        // }
        ...
        if ((*data)->Fu[ii] > 0)
        {
            ...
            if (param->superbee == 1 && (*data)->uu[smap->iMjc[ii]] != 0.0)   {
        ...
        else
        {
            ...
            if (param->superbee == 1 && smap->ii[ii] != param->nx-1 && (*data)->uu[smap->iPjc[ii]] != 0.0) {
        ...
        if ((*data)->Fu[smap->iMjc[ii]] > 0)
        {
            ...
            if (param->superbee == 1 && smap->ii[ii] != 0 && (*data)->uu[smap->iMjc[smap->iMjc[ii]]] != 0.0)   {
        ...
        else
        {
            ...
            if (param->superbee == 1 && (*data)->uu[ii] != 0.0)   {
        ...
        if ((*data)->Fv[ii] > 0)
        {
            ...
            if (param->superbee == 1 && (*data)->vv[smap->icjM[ii]] != 0.0)   {
        ...
        else
        {
            ...
            if (param->superbee == 1 && smap->jj[ii] != param->ny-1 && (*data)->vv[smap->icjP[ii]] != 0.0) {
        ...
        if ((*data)->Fv[smap->icjM[ii]] > 0)
        {
            ...
            if (param->superbee == 1 && smap->jj[ii] != 0 && (*data)->vv[smap->icjM[smap->icjM[ii]]] != 0.0)   {
        ...
        else
        {
            ...
            if (param->superbee == 1 && (*data)->vv[ii] != 0.0)   {
        ...
        if (param->sim_groundwater == 1)
        {
            ...
            if ((*data)->dept[ii] > 0.0)    {
                ...
                (*data)->qdifz[ii] = (2.0 * (*data)->Dzz[jj] / smap->dz[ii]) * ((*data)->s_surfkP[kk][ii] - (*data)->s_surf[kk][ii]);
            }
            else {
                (*data)->sseepage[kk][ii] = 0.0;
                (*data)->qdifz[ii] = 0.0;
            }
            // calculate total salt mass exchanged
            (*data)->sm_surf[kk][ii] += (*data)->sseepage[kk][ii] * param->dx * param->dy * param->dt;
            // if ((*data)->Asz[ii] > 0.0) {
            //     (*data)->sm_surf[kk][ii] += (*data)->sseepage[kk][ii] * (*data)->Asz[ii] * param->dt;
            // }
            // else {
            //     (*data)->sm_surf[kk][ii] += (*data)->sseepage[kk][ii] * param->dx * param->dy * param->dt;
            // }
        ...
        // wetting/drying
        if ((*data)->dept[ii] > 0 && (*data)->Vs[ii] == 0.0)   {
            int iwet;
            double s_min_wd = 0.0;
            // find the wetting source
            if ((*data)->uu[ii] != 0.0) {iwet = smap->iPjc[ii];}
            else if ((*data)->uu[smap->iMjc[ii]] != 0.0)    {iwet = smap->iMjc[ii];}
            else if ((*data)->vv[ii] != 0.0)    {iwet = smap->icjP[ii];}
            else if ((*data)->vv[smap->icjM[ii]] != 0.0)    {iwet = smap->icjM[ii];}
            if ((*data)->s_surf[kk][iwet] > s_min_wd) {
                s_min[ii] = (*data)->s_surf[kk][iwet];
            }
        }
        // scalar from subsurface
        ...
        // scalar from evaporation
        // if ((*data)->dept[ii] > param->hD && (*data)->evap[ii] != 0.0 && (*data)->Vflux[ii] > 0.0)  {
        if ((*data)->dept[ii] > param->hE && (*data)->evap[ii] != 0.0 && (*data)->Vflux[ii] > 0.0)  {
            s_max[ii] = s_max[ii]*(*data)->Vflux[ii] / ((*data)->Vflux[ii] - 1.2*param->dt*(*data)->evap[ii]*param->dx*param->dy);
            // if ((*data)->Asz[ii] <= 0.0)    {
            //     s_max[ii] = s_max[ii]*(*data)->Vflux[ii] / ((*data)->Vflux[ii] - 1.2*param->dt*(*data)->evap[ii]*param->dx*param->dy);
            // }
            // else {
            //     s_max[ii] = s_max[ii]*(*data)->Vflux[ii] / ((*data)->Vflux[ii] - 1.2*param->dt*(*data)->evap[ii]*(*data)->Asz[ii]);
            // }
        }
    }
    // update scalar
    for (ii = 0; ii < param->n2ci; ii++)
    {
        (*data)->s_surfn[kk][ii] = (*data)->s_surf[kk][ii];
        if ((*data)->dept[ii] > 0 & (*data)->Vs[ii] > 0)    {
            if (s_min[ii] > (*data)->s_surfn[kk][ii])   {s_min[ii] = (*data)->s_surfn[kk][ii];}
            if (s_max[ii] < (*data)->s_surfn[kk][ii])   {s_max[ii] = (*data)->s_surfn[kk][ii];}
        }
        ...
        {
            if ((*data)->dept[ii] > 0 & (*data)->Vs[ii] > 0)  {
                (*data)->s_surf[kk][ii] = (*data)->sm_surf[kk][ii] / (*data)->Vs[ii];
            }
            else {
                (*data)->s_surf[kk][ii] = 0.0;
            }
        }
        s_tmp = (*data)->s_surf[kk][ii];

        // if (smap->ii[ii] == 58 && smap->jj[ii] == 15)   {
        //     printf(" SUF : d=%f, (%f,%f,%f,%f), sn=%f, s_subs=%f, sm_seep=%f, q_seep=%f, sm=%f, Vf=%f, smax=%f ---> s=%f\n",
        //         (*data)->dept[ii],(*data)->s_surf[kk][smap->iPjc[ii]],(*data)->s_surf[kk][smap->iMjc[ii]],
        //         (*data)->s_surf[kk][smap->icjP[ii]],(*data)->s_surf[kk][smap->icjM[ii]],(*data)->s_surfn[kk][ii],
        //         (*data)->s_surfkP[kk][ii], (*data)->sseepage[kk][ii] * param->dx * param->dy * param->dt,
        //         (*data)->qss[ii] * param->dt * (*data)->Asz[ii],
        //         (*data)->sm_surf[kk][ii], (*data)->Vflux[ii], s_max[ii], (*data)->s_surf[kk][ii]);
        // }
        ...
        if ((*data)->dept[ii] > 0)
        {
            ...
            (*data)->sm_loss[0] += (s_tmp - (*data)->s_surf[kk][ii])*(*data)->Vflux[ii];

            // if (fabs(s_tmp - (*data)->s_surf[kk][ii])*(*data)->Vflux[ii] > 0.1)   {
            //
            //     printf(" (%d,%d) d=%f : mloss=%f, CFL=(%f,%f,%f,%f)\n",smap->ii[ii],smap->jj[ii],
            //         (*data)->dept[ii],(s_tmp - (*data)->s_surf[kk][ii])*(*data)->Vflux[ii],
            //         (*data)->uu[smap->iMjc[ii]]*(*data)->Asx[smap->iMjc[ii]]*param->dt/(*data)->Vsx[smap->iMjc[ii]],
            //         (*data)->uu[ii]*(*data)->Asx[ii]*param->dt/(*data)->Vsx[ii],
            //         (*data)->vv[smap->icjM[ii]]*(*data)->Asy[smap->icjM[ii]]*param->dt/(*data)->Vsy[smap->icjM[ii]],
            //         (*data)->vv[ii]*(*data)->Asy[ii]*param->dt/(*data)->Vsy[ii]);
            //     printf("         V=%f, Vn=%f, Vflux=%f, flux=(%f, %f, %f, %f)\n",(*data)->Vs[ii],(*data)->Vsn[ii],
            //         (*data)->Vflux[ii],(*data)->Fu[smap->iMjc[ii]]*param->dt,(*data)->Fu[ii]*param->dt,
            //         (*data)->Fv[smap->icjM[ii]]*param->dt,(*data)->Fv[ii]*param->dt);
            // }
        }

        // if (smap->jj[ii] == 35)  {
        //     if ((*data)->s_surf[kk][ii] < 40.0) {
        //         printf(" smax=%f, smin=%f\n",s_max[ii],s_min[ii]);
        //     }
        //     printf(" FINAL : (Vf=%f, V=%f, s=%f) -%f-  (Vf=%f, V=%f, s=%f, sm=%f) -%f- (Vf=%f, V=%f, s=%f)\n\n",
        //         (*data)->Vflux[ii-1],(*data)->Vs[ii-1],(*data)->s_surf[kk][ii-1],(*data)->vv[smap->icjM[ii]],
        //         (*data)->Vflux[ii],(*data)->Vs[ii],(*data)->s_surf[kk][ii],(*data)->sm_surf[kk][ii],
        //         (*data)->vv[ii],(*data)->Vflux[ii+1],(*data)->Vs[ii+1],(*data)->s_surf[kk][ii+1]);
        // }
...
void scalar_groundwater(Data **data, Map *gmap, Config *param, int irank, int nrank, int kk)
{
    ...
    double jip, jim, jjp, jjm, jkp, jkm, coeff, s_tmp;
    double *s_min, *s_max;
    s_lim_hi = 360.0;
    s_lim_lo = 0.0;
    ...
    for (aa = 0; aa < gmap->nactv; aa++)
    {
        ...
        if (gmap->jj[ii] == 0 & irank < param->mpi_nx) {jjm = 0.0;}
        ...
        else
        {
            ...
            // {jkm = 0.0;}
            // else
            // {jkm = dispersive_flux(data, gmap, param, ii, kk, "z");}
        }

        // (*data)->sm_subs[kk][ii] = (*data)->sm_subs[kk][ii] +
        //     param->dt * (*data)->r_rho[ii] * (*data)->wcs[ii] * ((jip - jim) + (jjp - jjm) + (jkp - jkm));
        (*data)->sm_subs[kk][ii] = (*data)->sm_subs[kk][ii] +
            param->dt * (*data)->wcs[ii] * ((jip - jim) + (jjp - jjm) + (jkp - jkm));
        ...
        if (param->sim_shallowwater == 1 & gmap->istop[ii] == 1)
        {
            ...
            if ((*data)->dept[gmap->top2d[ii]] > 0.0)   {
                (*data)->sm_subs[kk][ii] -= param->dt * gmap->Az[ii] * (*data)->sseepage[kk][gmap->top2d[ii]];
                // if ((*data)->Asz[gmap->top2d[ii]] > 0.0)    {
                //     (*data)->sm_subs[kk][ii] -= param->dt * (*data)->Asz[gmap->top2d[ii]] * (*data)->sseepage[kk][gmap->top2d[ii]];
                // }
                // else {
                //     (*data)->sm_subs[kk][ii] -= param->dt * gmap->Az[ii] * (*data)->sseepage[kk][gmap->top2d[ii]];
                // }
        ...
        // printf(" %d : End sm=%f, s=%f, V=%f, smax=%f\n\n",ii,(*data)->sm_subs[kk][ii],
        //     (*data)->sm_subs[kk][ii]/(*data)->Vgflux[ii], (*data)->Vgflux[ii],s_max[ii]);
    ...
    // {
        ...
        if (s_min[ii] > (*data)->s_subs[kk][ii])    {s_min[ii] = (*data)->s_subs[kk][ii];}
        if (s_max[ii] < (*data)->s_subs[kk][ii])    {s_max[ii] = (*data)->s_subs[kk][ii];}
        ...
        s_tmp = (*data)->s_subs[kk][ii];
        //
        // if (gmap->ii[ii] == 58 && gmap->jj[ii] == 15 && gmap->istop[ii] == 1)   {
        //     printf(" SUB : (%f,%f,%f,%f,%f,%f), sm_seep=%f, sm=%f, Vg=%f, smax=%f, smin=%f ---> s=%f\n\n",
        //         (*data)->s_subs[kk][gmap->iPjckc[ii]],(*data)->s_subs[kk][gmap->iMjckc[ii]],
        //         (*data)->s_subs[kk][gmap->icjPkc[ii]],(*data)->s_subs[kk][gmap->icjMkc[ii]],
        //         (*data)->s_subs[kk][gmap->icjckP[ii]],(*data)->s_subs[kk][gmap->icjckM[ii]],
        //         param->dt * gmap->Az[ii] * (*data)->sseepage[kk][gmap->top2d[ii]],
        //         (*data)->sm_subs[kk][ii], (*data)->Vgflux[ii], s_max[ii], s_min[ii], (*data)->s_subs[kk][ii]);
        // }
        ...
        if (param->sim_shallowwater == 1)
        {
            if (gmap->istop[ii] == 1 & (*data)->qz[gmap->icjckM[ii]] > 0 & (*data)->dept[gmap->top2d[ii]] <= 0.0)
            {
                s_max[ii] = s_max[ii] * (*data)->Vgflux[ii] / ((*data)->Vgflux[ii] - (*data)->qz[gmap->icjckM[ii]]*param->dt) + 0.05;
                // s_max[ii] += 0.5;
            }
        }

        // if (gmap->ii[ii] == 70 && gmap->jj[ii] == 95 && gmap->kk[ii] < 3)   {
        //     printf(" %d  :  wc=%f, sm=%f, Vf=%f, V=%f, qz=(%f,%f), q4=(%f,%f,%f,%f), smax=%f, smin=%f --> s=%f\n",
        //         gmap->kk[ii], (*data)->wc[ii], (*data)->sm_subs[kk][ii], (*data)->Vgflux[ii], (*data)->Vg[ii],
        //         (*data)->qz[ii],(*data)->qz[gmap->icjckM[ii]],(*data)->qx[ii],(*data)->qx[gmap->iMjckc[ii]],
        //         (*data)->qy[ii],(*data)->qy[gmap->icjMkc[ii]], s_max[ii], s_min[ii], (*data)->s_subs[kk][ii]);
        // }
        ...
        if ((*data)->Vgflux[ii] > 0 & (*data)->actv[ii] == 1)   {
            (*data)->sm_loss[1] += (s_tmp - (*data)->s_subs[kk][ii])*(*data)->Vgflux[ii];
            // if (fabs(s_tmp - (*data)->s_subs[kk][ii]) > 0.1)    {
            //     printf(" (%d,%d,%d) : d=%f, wc=%f, dz=%f, qz=(%f,%f), Vflux=%f, Vg=%f, s=%f, s_final=%f\n",gmap->ii[ii],gmap->jj[ii],gmap->kk[ii],
            //         (*data)->dept[gmap->top2d[ii]],(*data)->wc[ii],gmap->dz3d[ii],(*data)->qz[gmap->icjckM[ii]],(*data)->qz[ii],
            //         (*data)->Vgflux[ii],(*data)->Vg[ii],
            //         s_tmp, (*data)->s_subs[kk][ii]);
            // }
        }
        ...
        // update salt mass
        if ((*data)->Vg[ii] > 0 & gmap->actv[ii] == 1)  {
        ...
        // printf(" >> (%d,%d)  :  wc=%f, dept=%f, Final s = %f\n",gmap->jj[ii],gmap->kk[ii],(*data)->wc[ii],(*data)->dept[gmap->top2d[ii]],
        //     (*data)->s_subs[kk][ii]);
...
void advective_flux(Data **data, Map *gmap, Config *param, int icell, int kk)
{
    ...
    sip = 0.0;
    if ((*data)->qx[icell] < 0)    {
        sip = (*data)->s_subs[kk][icell];
        if (param->superbee == 1 && gmap->actv[gmap->iMjckc[icell]] == 1)   {
        ...
        if (param->superbee == 1 && gmap->ii[icell] != param->nx-1 && gmap->actv[gmap->iPjckc[gmap->iPjckc[icell]]] == 1)   {
    ...
    if ((*data)->qx[gmap->iMjckc[icell]] < 0)    {
        ...
        if (param->superbee == 1 && gmap->ii[icell] != 0 && gmap->actv[gmap->iMjckc[gmap->iMjckc[icell]]] == 1)  {
        ...
        sim = (*data)->s_subs[kk][icell];
        if (param->superbee == 1 && gmap->actv[gmap->iPjckc[icell]] == 1)  {
    ...
    sjp = 0.0;
    if ((*data)->qy[icell] < 0)    {
        sjp = (*data)->s_subs[kk][icell];
        if (param->superbee == 1 && gmap->actv[gmap->icjMkc[icell]] == 1)   {
        ...
        if (param->superbee == 1 && gmap->jj[icell] != param->ny-1 && gmap->actv[gmap->icjPkc[gmap->icjPkc[icell]]] == 1)   {
    ...
    if ((*data)->qy[gmap->icjMkc[icell]] < 0)    {
        ...
        if (param->superbee == 1 && gmap->jj[icell] != 0 && gmap->actv[gmap->icjMkc[gmap->icjMkc[icell]]] == 1)   {
        ...
        sjm = (*data)->s_subs[kk][icell];
        if (param->superbee == 1 && gmap->actv[gmap->icjPkc[icell]] == 1)   {
    ...
    if ((*data)->qz[icell] > 0)    {
        ...
        if (param->superbee == 1 && gmap->kk[icell] != param->nz-1 && gmap->actv[gmap->icjckP[gmap->icjckP[icell]]] == 1) {
        ...
        skp = (*data)->s_subs[kk][icell];
        if (param->superbee == 1 && gmap->actv[gmap->icjckM[icell]] == 1)   {
    ...
    // else {
    //     if ((*data)->qz[gmap->icjckM[icell]] < 0)   {skm = (*data)->s_subs[kk][gmap->icjckM[icell]];}
    //     else {skm = (*data)->s_subs[kk][icell];}
    // }
        ...
        if (gmap->actv[icell] == 1)
        {
            ...
            if ((*data)->Kz[icell] > 0 & gmap->actv[gmap->icjckP[icell]] == 1)
            {
                ...
                if (gmap->kk[icell] == param->nz-1)  {
                    fz = (*data)->Dzz[icell] * gmap->Az[icell] * ((*data)->s_subs[kk][gmap->icjckP[icell]] - (*data)->s_subs[kk][icell]) / (0.5 * gmap->dz3d[icell]);
                }
                else {
                    fz = (*data)->Dzz[icell] * gmap->Az[icell] * ((*data)->s_subs[kk][gmap->icjckP[icell]] - (*data)->s_subs[kk][icell]) /
                        (0.5 * (gmap->dz3d[icell] + gmap->dz3d[gmap->icjckP[icell]]));
                }
                // save diffusive exchange flux
                // if (gmap->kk[icell] == 0)    {
                //     (*data)->qdifz[gmap->top2d[icell]] = fz / gmap->Az[icell];
                // }
            }
        }
        else
...
void dispersion_tensor(Data **data, Map *gmap, Config *param)
{
    ...
    int ii;
    double q_abs, qx, qy, qz;
    for (ii = 0; ii < param->n3ct; ii++)
    {
        (*data)->Dxx[ii] = param->difux * (*data)->wc[ii];
        (*data)->Dxy[ii] = 0.0;
        (*data)->Dxz[ii] = 0.0;
        (*data)->Dyy[ii] = param->difuy * (*data)->wc[ii];
        (*data)->Dyx[ii] = 0.0;
        (*data)->Dyz[ii] = 0.0;
        (*data)->Dzz[ii] = param->difuz * (*data)->wc[ii];
        (*data)->Dzx[ii] = 0.0;
        (*data)->Dzy[ii] = 0.0;
    }
    for (ii = 0; ii < param->n3ct; ii++)
    {
        if (ii < param->n3ci)   {
            qx = (*data)->qx[ii] / gmap->Ax[ii];
            qy = (*data)->qy[ii] / gmap->Ay[ii];
        }
        else {
            qx = 0.0;   qy = 0.0;
        }
        qz = (*data)->qz[ii] / gmap->Az[ii];
        q_abs = pow(pow(qx,2.0) + pow(qy,2.0) + pow(qz,2.0), 0.5);

        if (q_abs > 0.0)
        {
            (*data)->Dxx[ii] += param->disp_lon * pow(qx,2.0) / q_abs + \
                param->disp_lat * pow(qy,2.0) / q_abs + \
                param->disp_lat * pow(qz,2.0) / q_abs;
            (*data)->Dxy[ii] += (param->disp_lon - param->disp_lat) * qx * qy / q_abs;
            (*data)->Dxz[ii] += (param->disp_lon - param->disp_lat) * qx * qz / q_abs;
            (*data)->Dyy[ii] += param->disp_lon * pow(qy,2.0) / q_abs + \
                param->disp_lat * pow(qx,2.0) / q_abs + \
                param->disp_lat * pow(qz,2.0) / q_abs;
            (*data)->Dyx[ii] += (param->disp_lon - param->disp_lat) * qx * qy / q_abs;
            (*data)->Dyz[ii] += (param->disp_lon - param->disp_lat) * qy * qz / q_abs;
            (*data)->Dzz[ii] += param->disp_lon * pow(qz,2.0) / q_abs + \
                param->disp_lat * pow(qy,2.0) / q_abs + \
                param->disp_lat * pow(qx,2.0) / q_abs;
            (*data)->Dzx[ii] += (param->disp_lon - param->disp_lat) * qx * qz / q_abs;
            (*data)->Dzy[ii] += (param->disp_lon - param->disp_lat) * qy * qz / q_abs;
        }
        ...
        // increase lateral dispersivity
        // if (gmap->istop[ii] == 1)   {
        //     (*data)->Dxx[ii] = (*data)->Dxx[ii] * 3.0;
        //     (*data)->Dyy[ii] = (*data)->Dyy[ii] * 3.0;
        // }
    }
}
