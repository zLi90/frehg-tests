...
void shallowwater_rhs(Data **data, Map *smap, Config *param)
{
    ...
    for (ii = 0; ii < param->n2ci; ii++)
    {
        ...
        // if (diff > param->hD)  {
        if (diff > param->hE)  {
            (*data)->Srhs[ii] -= (*data)->evap[ii] * (*data)->Asz[ii] * param->dt;
        }

        // subsurface
        // if (param->sim_groundwater == 1)    {
        //     (*data)->Srhs[ii] += (*data)->qss[ii] * (*data)->Asz[ii] * param->dt;
        // }
    }
...
void evaprain(Data **data, Map *smap, Config *param)
{
    ...
    for (ii = 0; ii < param->n2ci; ii++)
    {
        ...
        // apply evaporation over shallow layers
        // if (diff > param->hD)    {
        if (diff > param->hE)    {
            (*data)->eta[ii] -= (*data)->evap[ii] * param->dt;
        }
...
void volume_by_flux(Data **data, Map *smap, Config *param)
{
    ...
    for (ii = 0; ii < param->n2ci; ii++)
    {
        ...
        if (param->sim_groundwater == 1)
        {
            // if ((*data)->Asz[ii] > 0.0) {Aex = (*data)->Asz[ii];}
            // else {Aex = param->dx*param->dy;}
            Aex = param->dx*param->dy;

            if ((*data)->qss[ii] < 0)
            {(*data)->Vflux[ii] += (*data)->qss[ii] * param->dt * Aex;}
        ...
        // evaporation
        // if ((*data)->dept[ii] > param->hD)  {
        if ((*data)->dept[ii] > param->hE)  {
            (*data)->Vflux[ii] -= (*data)->evap[ii] * (*data)->Asz[ii] * param->dt;
        }
        ...
        if ((*data)->Vflux[ii] < 0.0)   {(*data)->Vflux[ii] = 0.0;}
    }
