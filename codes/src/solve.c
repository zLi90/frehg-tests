...
void solve(Data **data, Map *smap, Map *gmap, Config *param, int irank, int nrank)
{
    ...
    while (t_current < param->Tend)
    {
                ...
                for (ii = 0; ii < param->n3ci; ii++)    {
                    if (gmap->istop[ii] == 1)   {
                        ...
                        if ((*data)->eta[gmap->top2d[ii]] - (*data)->bottom[gmap->top2d[ii]] > 0.0)   {
                            ...
                            (*data)->dss[gmap->top2d[ii]] = fabs((param->dtg*(*data)->Dzz[jj]*
                                ((*data)->s_surf[0][gmap->top2d[ii]] - (*data)->s_surfn[0][gmap->top2d[ii]])/(param->dt*gmap->dz3d[ii])));
                        }
                    }
                }
            }
        ...
        if (param->sim_shallowwater == 1)   {update_subgrid_variable(data, smap, param, irank, nrank);}

        // monitor total mass conservation
        double ms = 0.0, mw = 0.0, mss = 0.0, mws = 0.0;
        if (param->n_scalar > 0 & param->sim_groundwater == 1)   {
            for (ii = 0; ii < param->n3ci; ii++)    {
                if (gmap->actv[ii] == 1)    {
                    ms += (*data)->sm_subs[0][ii];
                    mw += (*data)->wc[ii] * gmap->V[ii];
                }
            }
        }
        if (param->n_scalar > 0 & param->sim_shallowwater == 1)   {
            for (ii = 0; ii < param->n2ci; ii++)    {
                if ((*data)->dept[ii] > 0.0)    {
                    mss += (*data)->sm_surf[0][ii];
                    mws += (*data)->dept[ii] * param->dx * param->dy;
                }
            }
        }

        // write_monitor_out(mws, "mass_surf", 0, param);
        // write_monitor_out(mw, "mass_subs", 0, param);
        if (param->n_scalar > 0 & param->sim_shallowwater == 1) {
            write_monitor_out(mss, "saltmass_surf", 0, param);
            write_monitor_out((*data)->sm_loss[0], "saltloss_surf", 0, param);
        }
        if (param->n_scalar > 0 & param->sim_groundwater == 1)  {
            write_monitor_out(ms, "saltmass_subs", 0, param);
            write_monitor_out((*data)->sm_loss[1], "saltloss_subs", 0, param);
        }
        // monitor inundation area
        if (param->sim_shallowwater == 1)   {
            for (ii = 0; ii < param->n2ci; ii++)    {
                if ((*data)->dept[ii] > 0.0)    {(*data)->inun[ii] += 1.0;}
            }
        }
        // reset rainfall
            ...
            for (ii = 0; ii < param->n_monitor; ii++)
            {
                if (irank == (*data)->monitor_rank[ii])
                {
                    ...
                    if (param->sim_groundwater == 1)    {
                        ...
                        write_monitor_out(1e7*(*data)->dss[(*data)->monitor[ii]], "d_async", ii, param);
                        write_monitor_out(1e7*(*data)->qdifz[(*data)->monitor[ii]], "qdifz", ii, param);
                    }
                }
            }
        }
            ...
            for (ii = 0; ii < param->n3ci; ii++)
            {
                if (gmap->istop[ii] == 1)
                {
                    ...
                    else
                    {
                        ...
                        // dry limiter
                        if ((*data)->wc[ii] <= param->wcr+0.01)  {(*data)->evap[jj] = 0.0;}
                        // for ex4 - Toy delta
