...
void write_output(Data **data, Map *gmap, Config *param, int tt, int root, int irank)
{
    ...
    else
    {
        if (param->sim_shallowwater == 1)
        {
            for (ii = 0; ii < param->n2ci; ii++)
            {
                ...
                if (param->sim_groundwater == 1)
                {
                    (*data)->seep_out[ii] = (*data)->qss[ii]*8.64e7;
                    // (*data)->seep_out[ii] = (*data)->sseepage[0][ii]*8.64e7;
                    (*data)->rss[ii] = (*data)->rss[ii]*8.64e7;
                }
                if (param->n_scalar > 0)
                {
                    for (kk = 0; kk < param->n_scalar; kk++)
        ...
        if (param->sim_groundwater == 1)
        {
            for (ii = 0; ii < param->n3ci; ii++)
            {
                ...
                (*data)->qx_out[ii] = (*data)->qx[ii]*8.64e7/gmap->Ax[ii];
                (*data)->qy_out[ii] = (*data)->qy[ii]*8.64e7/gmap->Ay[ii];
                (*data)->qz_out[ii] = (*data)->qz[ii]*8.64e7/gmap->Az[ii];
                if (param->n_scalar > 0)
                {
                    for (kk = 0; kk < param->n_scalar; kk++)
    ...
    if (param->sim_shallowwater == 1 & irank == root)
    {
        ...
        write_one_file((*data)->inun, "inun", param, tt, param->N2CI);

        if (param->sim_groundwater == 1)
        {
            write_one_file((*data)->seep_out, "seepage", param, tt, param->N2CI);
            write_one_file((*data)->rss, "r_async", param, tt, param->N2CI);
        }
        if (param->n_scalar > 0)
        {
            for (kk = 0; kk < param->n_scalar; kk++)
