...
void adaptive_time_step(Data *data, Map *gmap, Config **param, int root, int irank)
{
    ...
    for (ii = 0; ii < (*param)->n3ci; ii++)
    {
        if (gmap->actv[ii] == 1 & gmap->istop[ii] == 0)
        {
            ...
            dq = fabs(data->qx[ii] - data->qx[gmap->iMjckc[ii]]) * (*param)->dt / (*param)->dx;
            dq += fabs(data->qy[ii] - data->qy[gmap->icjMkc[ii]]) * (*param)->dt / (*param)->dy;
            dq += fabs(data->qz[ii] - data->qz[gmap->icjckM[ii]]) * (*param)->dt / gmap->dz3d[ii];
            if (dq > dq_max)    {
    ...
    else if (dq_max < 0.015) {(*param)->dt = (*param)->dt * r_inc;}
